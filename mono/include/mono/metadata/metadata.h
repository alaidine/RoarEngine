...
MONO_API MONO_RT_EXTERNAL_ONLY MonoClass **mono_metadata_interfaces_from_typedef(MonoImage *meta, uint32_t table_index,
                                                                                 unsigned int *count);
...
MONO_API void mono_metadata_field_info(MonoImage *meta, uint32_t table_index, uint32_t *offset, uint32_t *rva,
...
#define MONO_OFFSET_IN_FILTER(clause, offset)                                                                                    \
    ((clause)->flags == MONO_EXCEPTION_CLAUSE_FILTER && (clause)->data.filter_offset <= (offset) &&                              \
...
MONO_API MONO_RT_EXTERNAL_ONLY MonoType *mono_signature_get_return_type(MonoMethodSignature *sig);

MONO_API MONO_RT_EXTERNAL_ONLY MonoType *mono_signature_get_params(MonoMethodSignature *sig, void **iter);
...
MONO_API MONO_RT_EXTERNAL_ONLY MonoType *mono_metadata_parse_type(MonoImage *m, MonoParseTypeMode mode, short opt_attrs,
                                                                  const char *ptr, const char **rptr);
MONO_API MONO_RT_EXTERNAL_ONLY MonoType *mono_metadata_parse_param(MonoImage *m, const char *ptr, const char **rptr);
MONO_API MONO_RT_EXTERNAL_ONLY MonoType *mono_metadata_parse_field_type(MonoImage *m, short field_flags, const char *ptr,
                                                                        const char **rptr);
...
MONO_API MONO_RT_EXTERNAL_ONLY MonoMethodSignature *mono_metadata_parse_method_signature(MonoImage *m, int def, const char *ptr,
                                                                                         const char **rptr);
...
MONO_API int mono_method_header_get_clauses(MonoMethodHeader *header, MonoMethod *method, void **iter,
                                            MonoExceptionClause *clause);

MONO_API uint32_t mono_type_to_unmanaged(MonoType *type, MonoMarshalSpec *mspec, mono_bool as_field, mono_bool unicode,
