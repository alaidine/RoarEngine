...
//  [!] Renderer: User texture binding. Use 'VkDescriptorSet' as texture identifier. Call ImGui_ImplVulkan_AddTexture() to
//  register one. Read the FAQ about ImTextureID/ImTextureRef + https://github.com/ocornut/imgui/pull/914 for discussions. [X]
//  Renderer: Large meshes support (64k+ vertices) even with 16-bit indices (ImGuiBackendFlags_RendererHasVtxOffset). [X]
//  Renderer: Texture updates support for dynamic font atlas (ImGuiBackendFlags_RendererHasTextures). [X] Renderer: Expose
//  selected render state for draw callbacks to use. Access in
//  '(ImGui_ImplXXXX_RenderState*)GetPlatformIO().Renderer_RenderState'. [x] Renderer: Multi-viewport / platform windows. With
//  issues (flickering when creating a new viewport).
...
//  init_info.PipelineInfoMain.RenderPass, init_info.Subpass --> init_info.PipelineInfoMain.Subpass, init_info.MSAASamples -->
...
//  ImGui_ImplVulkan_MainPipelineCreateInfo to ImGui_ImplVulkan_PipelineInfo. Introduced very recently so shouldn't affect many
//  users. 2025-09-26: *BREAKING CHANGE*: helper ImGui_ImplVulkanH_CreateOrResizeWindow() added a VkImageUsageFlags image_usage`
//  argument, default to VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT if 0. 2025-09-26: Vulkan: Added a way to customize shaders by filling
//  ImGui_ImplVulkan_InitInfo::CustomShaderVertCreateInfo/CustomShaderFragCreateInfo. (#8585) 2025-09-22: [Docking] Added
//  ImGui_ImplVulkanH_GetWindowDataFromViewport() accessor/helper. (#8946, #8940) 2025-09-18: Call
//  platform_io.ClearRendererHandlers() on shutdown. 2025-09-04: Vulkan: Added ImGui_ImplVulkan_CreateMainPipeline(). (#8110,
//  #8111) 2025-07-27: Vulkan: Fixed texture update corruption introduced on 2025-06-11. (#8801, #8755, #8840) 2025-07-07: Vulkan:
//  Fixed texture synchronization issue introduced on 2025-06-11. (#8772) 2025-06-27: Vulkan: Fixed validation errors during
//  texture upload/update by aligning upload size to 'nonCoherentAtomSize'. (#8743, #8744) 2025-06-11: Vulkan: Added support for
//  ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. Removed ImGui_ImplVulkan_CreateFontsTexture() and
//  ImGui_ImplVulkan_DestroyFontsTexture(). 2025-05-07: Vulkan: Fixed validation errors during window detach in multi-viewport
//  mode. (#8600, #8176) 2025-05-07: Vulkan: Load dynamic rendering functions using vkGetDeviceProcAddr() + try both non-KHR and
//  KHR versions. (#8600, #8326, #8365) 2025-04-07: Vulkan: Deep-copy ImGui_ImplVulkan_InitInfo::PipelineRenderingCreateInfo's
//  pColorAttachmentFormats buffer when set, in order to reduce common user-error of specifying a pointer to data that gets out of
//  scope. (#8282) 2025-02-14: *BREAKING CHANGE*: Added uint32_t api_version to ImGui_ImplVulkan_LoadFunctions(). 2025-02-13:
//  Vulkan: Added ApiVersion field in ImGui_ImplVulkan_InitInfo. Default to header version if unspecified. Dynamic rendering path
//  loads "vkCmdBeginRendering/vkCmdEndRendering" (without -KHR suffix) on API 1.3. (#8326) 2025-01-09: Vulkan: Added
//  IMGUI_IMPL_VULKAN_MINIMUM_IMAGE_SAMPLER_POOL_SIZE to clarify how many image sampler descriptors are expected to be available
//  in descriptor pool. (#6642) 2025-01-06: Vulkan: Added more ImGui_ImplVulkanH_XXXX helper functions to simplify our examples.
//  2024-12-11: Vulkan: Fixed setting VkSwapchainCreateInfoKHR::preTransform for platforms not supporting
//  VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR. (#8222) 2024-11-27: Vulkan: Make user-provided descriptor pool optional. As a
//  convenience, when setting init_info->DescriptorPoolSize the backend will create one itself. (#8172, #4867) 2024-10-07: Vulkan:
//  Changed default texture sampler to Clamp instead of Repeat/Wrap. 2024-10-07: Vulkan: Expose selected render state in
//  ImGui_ImplVulkan_RenderState, which you can access in 'void* platform_io.Renderer_RenderState' during draw callbacks.
//  2024-10-07: Vulkan: Compiling with '#define ImTextureID=ImU64' is unnecessary now that dear imgui defaults ImTextureID to u64
//  instead of void*. 2024-04-19: Vulkan: Added convenience support for Volk via IMGUI_IMPL_VULKAN_USE_VOLK define (you can also
//  use IMGUI_IMPL_VULKAN_NO_PROTOTYPES + wrap Volk via ImGui_ImplVulkan_LoadFunctions().) 2024-02-14: *BREAKING CHANGE*: Moved
//  RenderPass parameter from ImGui_ImplVulkan_Init() function to ImGui_ImplVulkan_InitInfo structure. Not required when using
//  dynamic rendering. 2024-02-12: *BREAKING CHANGE*: Dynamic rendering now require filling PipelineRenderingCreateInfo structure.
//  2024-01-19: Vulkan: Fixed vkAcquireNextImageKHR() validation errors in VulkanSDK 1.3.275 by allocating one extra semaphore
//  than in-flight frames. (#7236) 2024-01-11: Vulkan: Fixed vkMapMemory() calls unnecessarily using full buffer size (#3957).
//  Fixed MinAllocationSize handing (#7189). 2024-01-03: Vulkan: Added MinAllocationSize field in ImGui_ImplVulkan_InitInfo to
//  workaround zealous "best practice" validation layer. (#7189, #4238) 2024-01-03: Vulkan: Stopped creating command pools with
//  VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT as we don't reset them. 2023-11-29: Vulkan: Fixed mismatching allocator passed
//  to vkCreateCommandPool() vs vkDestroyCommandPool(). (#7075) 2023-11-10: *BREAKING CHANGE*: Removed parameter from
...
//  end of render a full-viewport to reduce likelihood of issues with people using VK_DYNAMIC_STATE_SCISSOR in their app without
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback != nullptr) {
                ...
                if (desc_set != last_desc_set)
                    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, bd->PipelineLayout, 0, 1, &desc_set,
                                            0, nullptr);
...
void ImGui_ImplVulkan_UpdateTexture(ImTextureData *tex) {
    ...
    if (tex->Status == ImTextureStatus_WantCreate) {
        ...
        backend_tex->DescriptorSet =
            ImGui_ImplVulkan_AddTexture(bd->TexSamplerLinear, backend_tex->ImageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    ...
    if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates) {
        ...
        // Copy to Image:
        {
            ...
            vkCmdCopyBufferToImage(bd->TexCommandBuffer, upload_buffer, backend_tex->Image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                   1, &region);
            ...
            vkCmdPipelineBarrier(bd->TexCommandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0,
...
static void ImGui_ImplVulkan_CreateShaderModules(VkDevice device, const VkAllocationCallbacks *allocator) {
    ...
    if (bd->ShaderModuleVert == VK_NULL_HANDLE) {
        ...
        VkShaderModuleCreateInfo *p_vert_info =
            (v->CustomShaderVertCreateInfo.sType == VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO) ? &v->CustomShaderVertCreateInfo
                                                                                                 : &default_vert_info;
    ...
    if (bd->ShaderModuleFrag == VK_NULL_HANDLE) {
        ...
        VkShaderModuleCreateInfo *p_frag_info =
            (v->CustomShaderFragCreateInfo.sType == VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO) ? &v->CustomShaderFragCreateInfo
                                                                                                 : &default_frag_info;
...
static void ImGui_ImplVulkan_LoadDynamicRenderingFunctions(
    ...
    // - Fallback to KHR versions if core not available (this will work if KHR extension is available and enabled and also the
...
// We don't care about other versions than 1.3 for our checks, so don't need to make this exhaustive (e.g. with all #ifdef
// VK_VERSION_1_X checks)
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo *info) {
    ...
    if (info->UseDynamicRendering) {
        ...
        ImGui_ImplVulkan_LoadDynamicRenderingFunctions(
            info->ApiVersion,
            [](const char *function_name, void *user_data) { return vkGetDeviceProcAddr((VkDevice)user_data, function_name); },
            (void *)info->Device);
#endif
    ...
    // Our render function expect RendererUserData to be storing the window render buffer we need (for the main viewport we won't
    // use ->Window)
...
// You may read this code to learn about Vulkan, but it is recommended you use your own custom tailored code to do equivalent
...
void ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance instance, VkPhysicalDevice physical_device, VkDevice device,
    ...
    // FIXME: to submit the command buffer, we need a queue. In the examples folder, the ImGui_ImplVulkanH_CreateOrResizeWindow
    // function is called before the ImGui_ImplVulkan_Init function, so we don't have access to the queue yet. Here we have the
    // queue_family that we can use to grab a queue from the device and submit the command buffer. It would be better to have
...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport *viewport) {
    ...
    if (res != VK_TRUE) {
        vkDestroySurfaceKHR(v->Instance, wd->Surface,
                            v->Allocator); // Error: no WSI support on physical device, clean up and return
        IM_DELETE(vd);
        return;
    }
    ...
    wd->SurfaceFormat = ImGui_ImplVulkanH_SelectSurfaceFormat(v->PhysicalDevice, wd->Surface, requestSurfaceImageFormats.Data,
                                                              (size_t)requestSurfaceImageFormats.Size, requestSurfaceColorSpace);

    // Select Present Mode
    // FIXME-VULKAN: Even thought mailbox seems to get us maximum framerate with a single window, it halves framerate with a
    // second window etc. (w/ Nvidia and SDK 1.82.1)
    ...
    if (bd->PipelineForViewports == VK_NULL_HANDLE) {
        ...
        bd->PipelineForViewports =
            ImGui_ImplVulkan_CreatePipeline(v->Device, v->Allocator, VK_NULL_HANDLE, &v->PipelineInfoForViewports);
    }
}
...
static void ImGui_ImplVulkan_SetWindowSize(ImGuiViewport *viewport, ImVec2 size) {
    ...
    ImGui_ImplVulkanH_CreateOrResizeWindow(v->Instance, v->PhysicalDevice, v->Device, &vd->Window, v->QueueFamily, v->Allocator,
...
static void ImGui_ImplVulkan_RenderWindow(ImGuiViewport *viewport, void *) {
    ...
    ImGui_ImplVulkanH_FrameSemaphores *fsd = &wd->FrameSemaphores[wd->SemaphoreIndex];
    {
        ...
        if (v->UseDynamicRendering) {
            ...
            barrier.subresourceRange.levelCount = 1;
            barrier.subresourceRange.layerCount = 1;
            vkCmdPipelineBarrier(fd->CommandBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_NONE,
