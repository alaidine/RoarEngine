...
void UIOverlay::prepareResources() {
    ...
    VkDescriptorPoolCreateInfo descriptorPoolInfo{
        .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO, .maxSets = 2, .poolSizeCount = 1, .pPoolSizes = &poolSize};
    ...
    VkDescriptorSetLayoutCreateInfo descriptorLayout{
        .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO, .bindingCount = 1, .pBindings = &setLayoutBinding};
    ...
    VkDescriptorImageInfo fontDescriptor{
        .sampler = sampler, .imageView = fontView, .imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
 ...
 * main application */
void UIOverlay::preparePipeline(const VkPipelineCache pipelineCache, const VkRenderPass renderPass, const VkFormat colorFormat,
    ...
    // Enable blending
    VkPipelineColorBlendAttachmentState blendAttachmentState{.blendEnable = VK_TRUE,
    ...
    // If we are using dynamic rendering (renderPass is null), we must define
...
void UIOverlay::update(uint32_t currentBuffer) {
    ...
    // Create buffers with multiple of a chunk size to minimize the need to
    // recreate them
    ...
    if ((buffers[currentBuffer].vertexBuffer.buffer == VK_NULL_HANDLE) ||
        (buffers[currentBuffer].vertexBuffer.size < vertexBufferSize)) {
        ...
        VK_CHECK_RESULT(device->createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
                                             &buffers[currentBuffer].vertexBuffer, vertexBufferSize));
    ...
    if ((buffers[currentBuffer].indexBuffer.buffer == VK_NULL_HANDLE) ||
        (buffers[currentBuffer].indexBuffer.size < indexBufferSize)) {
        ...
        VK_CHECK_RESULT(device->createBuffer(VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
                                             &buffers[currentBuffer].indexBuffer, indexBufferSize));
...
void UIOverlay::draw(const VkCommandBuffer commandBuffer, uint32_t currentBuffer) {
    ...
    if (buffers[currentBuffer].vertexBuffer.buffer == VK_NULL_HANDLE ||
        buffers[currentBuffer].indexBuffer.buffer == VK_NULL_HANDLE) {
        return;
    }
    ...
    assert(buffers[currentBuffer].vertexBuffer.buffer != VK_NULL_HANDLE &&
           buffers[currentBuffer].indexBuffer.buffer != VK_NULL_HANDLE);
...
bool UIOverlay::header(const char *caption) { return ImGui::CollapsingHeader(caption, ImGuiTreeNodeFlags_DefaultOpen); }

bool UIOverlay::checkBox(const char *caption, bool *value) { return ImGui::Checkbox(caption, value); }
